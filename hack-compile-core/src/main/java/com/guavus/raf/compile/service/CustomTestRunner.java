...
public class CustomTestRunner {
    ...
    public List<CompileTestResponse> invokeTest(UDFCompute udfCompute, UDFComputeTest testObj) {
        Set<String> methodTest = testObj.testSet();
        List<CompileTestResponse> testResponses = new ArrayList<>();
        for (String methodName : methodTest) {
            try {
                ...
                if (result) {
                    testResponses.add(new CompileTestResponse(methodName, "SUCCESS"));
                } else {
                    testResponses.add(new CompileTestResponse(methodName, "FAILED"));
                }
            } catch (Exception e) {
                e.printStackTrace();
                testResponses.add(new CompileTestResponse(methodName, ExceptionUtils.getStackTrace(e)));
            }
        }
